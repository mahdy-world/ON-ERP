...
class Invoice(models.Model):
    ...
    spend_category = models.ForeignKey(SpendCategory, on_delete=models.SET_NULL, null=True, blank=True,verbose_name='التصنيف')
    ...
    from_branch = models.ForeignKey(Branch, verbose_name='من فرع', on_delete=models.SET_NULL, null=True, related_name='branch_out_transactions')
    to_branch = models.ForeignKey(Branch, verbose_name='إلي فرع', on_delete=models.SET_NULL, null=True, related_name='branch_in_transactions')
    from_warehouse = models.ForeignKey(Warehouse, on_delete=models.SET_NULL, null=True, blank=True,verbose_name='من مخزن', related_name='warehouse_out_transactions')
    to_warehouse = models.ForeignKey(Warehouse, on_delete=models.SET_NULL, null=True, blank=True,verbose_name='الي مخزن', related_name='warehouse_in_transactions')
    ...
    main_warehouse = models.ForeignKey(Warehouse, on_delete=models.SET_NULL, null=True, blank=True,verbose_name='المخزن الرئيسي', related_name='invoice_main_warehouse')
    sub_warehouse = models.ForeignKey(Warehouse, on_delete=models.SET_NULL, null=True, blank=True,verbose_name='المخزن الفرعي', related_name='invoice_sub_warehouse')
    ...
    def calculate_user_debit_credit(self):
        if self.invoice_type in [1, 2, 3, 6, 21]:
            self.customer_debit = self.treasury_in
            self.customer_credit = self.overall
        if self.invoice_type in [4, 5, 22]:
            self.customer_credit = self.treasury_out
            self.customer_debit = self.overall
        self.save()
    ...
    def calculate_profit(self):
        if self.invoice_type in [1, 2, 3, 6, 21]:
            ...
            for x in self.invoiceitem_set.all():
                self.profit += x.profit
            self.profit -= self.invoice_discount
        if self.invoice_type in [4, 5, 22]:
            self.profit = self.discount + self.invoice_discount
        self.save()
    ...
    def calculate_over_all(self):
        self.overall = self.after_discount - self.invoice_discount + self.shipping
        self.save()
...
class InvoiceItem(models.Model):
    ...
    main_unit_price = models.FloatField(default=0.0, verbose_name='سعر الوحدة الرئيسية')
    unit_price = models.FloatField(default=0.0, verbose_name='سعر الوحدة المختارة')
    ...
    def calculate(self):
        ...
        self.calculate_main_units()

    def calculate_profit(self):
        if self.invoice.invoice_type in [1, 2, 3, 6, 21]:
            self.profit = 0
        if self.invoice.invoice_type in [4, 5, 22]:
            self.profit = self.discount
        self.save()

    # def calculate_profit(self):
    #     self.cost_profit = self.after_discount - (self.item.cost_price * self.main_unit_quantity)
    #     self.purchase_profit = self.after_discount - (self.item.purchase_price * self.main_unit_quantity)
    #     self.save()
...
class WarehouseTransactions(models.Model):
    ...
    item = models.ForeignKey(Product, on_delete=models.SET_NULL, null=True, verbose_name='المنتج')
    incoming = models.FloatField(default=0.0, verbose_name='كمية الوارد')
    purchase_cost = models.FloatField(default=0.0, verbose_name='سعر الشراء')
    total_incoming = models.FloatField(default=0.0, verbose_name='قيمة إجمالي الوارد')
    outgoing = models.FloatField(default=0.0, verbose_name='كمية المنصرف')
...
class TreasuryTransactions(models.Model):
